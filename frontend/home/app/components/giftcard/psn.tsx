...
const PSNGiftCardSection: React.FC = () => {
  const [psnGiftCards, setPSNGiftCards] = useState<PSNGiftCard[]>([]);

  useEffect(() => {
    const fetchPSNGiftCards = async () => {
      try {
        const response = await axios.get('/api/psn-gift-cards');
        setPSNGiftCards(response.data);
      } catch (error) {
        console.error('Error fetching PSN gift cards:', error);
      }
    };
    ...
    <div className="bg-black-100 py-10 lg:py-20">
      <div className="container">
        ...
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold mb-2">PSN Gift Cards</h2>
          <p className="text-gray-500">Browse our selection of PSN gift cards.</p>
        </div>
          ...
          {psnGiftCards.map((card) => (
            ...
            >
              <div className="h-[100px]">
                <img
                  src={card.image}
                  alt={`PSN Gift Card - $${card.denomination}`}
...
export default PSNGiftCardSection;